...
export default function CustomTable<Data>({ baseUrl, title, columns, selectFilters }: CustomTableProps<Data>) {
  ...
  const updateFilters = useCallback((e: { target: { name: string; value: string } }) => {
  ...
  useEffect(() => {
    setFilters((prev) => ({ ...prev, limit: pageSize, offset: pageIndex * pageSize }));
  }, [pageIndex, pageSize]);
    ...
    <Card
      ...
      headerslot={
        <div className="items-center flex gap-2">
          <input className="p-2" name="search" placeholder="Search" onChange={updateFilters} />
          {!!selectFilters?.length && <Button className="btn btn-dark py-2">Filter</Button>}
        </div>
      }>
            ...
            <table className="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                ...
                {headerGroups.map((headerGroup) => (
                  <tr>
                    {headerGroup.headers.map((header) => (
                      <th key={header.id} colSpan={header.colSpan} scope="col" className=" table-th ">
              ...
              <tbody className="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={99}>
                      <p className="text-center py-6">We couldn't load the data</p>
                    </td>
                  </tr>
                ) : data?.results?.length ? (
